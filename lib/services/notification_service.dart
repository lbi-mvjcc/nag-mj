...
class NotificationService {
  ...
  bool _hasBundledWindowsAlarmSound = false;
  ...
  static const int _alarmRepeatCount = 3;
  static const Duration _windowsExactAlarmGrace = Duration(seconds: 2);
  ...
  Future<void> setWindowsReminderSoundEnabled(bool enabled) async {
    ...
    await prefs.setBool(AppConstants.windowsReminderSoundEnabledKey, enabled);
  }
  ...
  Future<void> _scheduleOneTimeNotification(
    ...
    print(
      'Scheduled one-time notification ($offsetMinutes min) for: $scheduledDate',
    );
  }
  ...
  Future<void> rescheduleAllNotifications(List<Task> tasks) async {
    ...
    final pendingTasks = tasks
        .where((task) => task.reminderDateTime != null && !task.isCompleted)
        .toList();
  ...
  Future<void> _scheduleWindowsNotification(Task task) async {
    ...
    for (var index = 0; index < _alertOffsetsInMinutes.length; index++) {
      ...
      final isExactAlarm = offset == 0;

      if (alertTime.isBefore(now.subtract(_windowsNearDueThreshold))) {
        continue;
      }

      if (!isExactAlarm && delay <= _windowsNearDueThreshold) {
      ...
      if (isExactAlarm && delay <= _windowsExactAlarmGrace) {
        await _showWindowsNotification(task, nextTrigger, offset);
        if (task.isRecurring && task.recurrenceType != RecurrenceType.none) {
          _scheduleNextRecurringWindowsNotification(task, nextTrigger);
        }
  ...
  void _scheduleNextRecurringWindowsNotification(
    Task task,
    DateTime occurrence,
  ) {
  ...
  Future<void> _showWindowsNotification(
    ...
    // Determine if this is the main alarm (exact time) or a reminder
    final isMainAlarm = offsetMinutes == 0;
    final shouldPlaySound = _isWindowsReminderSoundEnabled;
    final shouldPlayCustomAlarmSound = isMainAlarm && shouldPlaySound;
    final shouldMuteToastSound = isMainAlarm ? true : !shouldPlaySound;

    Future<void> showNotification() async {
      final localNotification = LocalNotification(
        title: isMainAlarm ? 'Task Alarm' : 'Task Reminder',
        body:
            '${_withAlertOffsetBody(task.title, offsetMinutes)}\nScheduled: ${scheduledFor.toLocal()}',
        silent: shouldMuteToastSound,
      );
      // Stop any custom alarm sound when the user clicks the notification.
      ...
      if (shouldPlayCustomAlarmSound) {
        await _playWindowsAlarmSound();
      }
    }
  ...
  Future<void> _validateBundledWindowsReminderSound() async {
    ...
    // Check for alarm sound (for exact-time alerts)
    try {
      await rootBundle.load(
        'assets/${AppConstants.windowsAlarmSoundAssetPath}',
      );
      _hasBundledWindowsAlarmSound = true;
    } catch (e) {
      _hasBundledWindowsAlarmSound = false;
      debugPrint(
        'Bundled alarm sound not found at assets/${AppConstants.windowsAlarmSoundAssetPath}: $e',
      );
    }
  }
  ...
  Future<void> _playWindowsAlarmSound() async {
    if (!Platform.isWindows) {
      return;
    }

    try {
      // Play alarm sound multiple times with short delays for more urgency
      for (int i = 0; i < _alarmRepeatCount; i++) {
        await _windowsAudioPlayer.stop();
        await _windowsAudioPlayer.play(
          AssetSource(AppConstants.windowsAlarmSoundAssetPath),
        );

        // Wait for sound to play + brief delay before next repetition
        if (i < _alarmRepeatCount - 1) {
          await Future.delayed(const Duration(milliseconds: 500));
        }
      }
    } catch (e) {
      debugPrint('Bundled Windows alarm sound playback failed: $e');
    }
  }
  ...
  int _notificationIdFor(int taskId, int alertIndex) {
    return AppConstants.defaultNotificationIdOffset +
        (taskId * 10) +
        alertIndex;
  }
